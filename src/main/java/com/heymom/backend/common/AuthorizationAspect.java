	...
	private String getUserToken(JoinPoint joinPoint) throws Exception {
		...
		for (int i = 0; i < annotations.length; i++) {
			for (Annotation parameterAnnotation : annotations[i]) {
				if (parameterAnnotation.annotationType().equals(RequestHeader.class)) {
					if ("token".equals(parameterAnnotation.annotationType().getMethod("value")
							.invoke(parameterAnnotation).toString())) {
